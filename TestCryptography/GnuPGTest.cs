...
namespace TestCryptography
{
    ...
    {
        private const string hermesPublicKeyID = "79222C24";
        private const string hermesPublicSubKeyID = "51649C30";
...
-----END PGP PUBLIC KEY BLOCK-----";
        ...
        public void ListKeysTest()
        {
            ...
            GnuPGKey puttyKey = keys.FirstOrDefault<GnuPGKey>(s => s.Key == puttyPublicKeyID);
            ...
            GnuPGKey hermesKey = keys.FirstOrDefault<GnuPGKey>(s => s.Key == hermesPublicKeyID);
            ...
            Assert.IsTrue(hermesSubKey.Key == hermesPublicSubKeyID);
        }
        ...
        public void ImportGoodKeyTest()
        {
            ...
            GnuPGKey key = gpg.GetKeys().FirstOrDefault(s => s.Key == puttyPublicKeyID);
            if (key != null) 
            {
                gpg.DeleteKey(key.Key);
                // Verify it isn't there
                key = gpg.GetKeys().FirstOrDefault(s => s.Key == puttyPublicKeyID);
                Assert.IsNull(key);
            }
            ...
            string fingerprint = gpg.GetFingerprint(key.Key);
